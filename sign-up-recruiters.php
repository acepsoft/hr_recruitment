                ...
                <div class="col-lg-6">
                    <?php if (!isset($responseData['success']) || !$responseData['success']): ?>
                        <form action="" method="post" id="registration-form" onsubmit="return validateForm();">
                            <input type="hidden" name="action" value="saverecruiter">
                            <div class="mb-3">
                                <label for="name" class="form-label text-start w-100">Name</label>
                                <input type="text" class="form-control" id="name" name="name" required value="<?php echo htmlspecialchars($oldValues['name'] ?? '', ENT_QUOTES); ?>">
                            </div>
